...
//            var_dump($channelList);die;
	...
	**/
    ...
    public function getTimePayment($channelid,$dailiname,$starttime,$endtime){
	...
	**/
	public function getNextRemainMoney(){
		if(cookie('uid')>0&&cookie('channelid')>0){
			...
			$data = $this->getTimePayment($channelid,$dailiname,$starttime,$endtime);
...
/****更新充值笔数****/
	...
	public function upisall(){
		$data = M('distribution_payment')
				->field('sum(askmoney) as sumaskmoney,channelid')
				->group('channelid')
				->select();
				$model =  M('distribution_channels');
				$fo = array();
		foreach($data as $k=>$v){
			$dats = $model->field('pid')->where(array('channelid'=>$v['channelid']))->find();
			if($dats['pid']==0){
				$dat['isall'] = 1;
			}else{
				$dat['isall'] =0 ;
			...
			$res = M('distribution_payment')->where(array('channelid'=>$v['channelid']))->save($dat);
		}		
	}
	/***
	*ajax获取下级结算信息
			...
			if(!empty($channelid)){	
				...
				//var_dump($channelInfo);exit;
					...
					if($iswhen==''){
						...
						$data = $channelModel->getNextMoney($channelid,1,$issum,15,$starttime,$endtime,$dailiname,$isoverpay,$iswhen);
						//var_dump($data);exit;
						...
						foreach ($data['data'] as $k=>$v){
			                ...
			                /*渠道财务信息*/
				           /* $bankInfo=$channelModel->getChannelBankinfo($v['channelid']);
				            $bankInfo['uname'] = $userInfo['uname'];			
				            $data['data'][$k]['bankInfo']=$bankInfo;*/
			            }
