...
class ChannelModel extends Model
{
    ...
    *获取子渠道结算信息
    **/
    public function getNextMoney($channelid,$ischannel=0,$issum=0,$pageSize=15){
        header('content-type:text/html;charset=utf-8');
        $todayTime=strtotime(date('Y-m-d'));
        $beforeTime=$todayTime-30*24*60*60;
        if($ischannel==1 || $issum==1){
        ...
        $where['payflag']=1;
        $count = M('distribution_pay_paylog')->where($where)->group('channelid,FROM_UNIXTIME(buytime,"%Y-%m-%d")')->select();
        //echo M()->getLastSql();exit;
        //var_dump($count);exit;
      //
        $count=count($count);
        ...
        $data['data']=M('distribution_pay_paylog')
            ...
            ->where(array('payflag'=>1,'channelid'=>array('IN',$channelidArr),'buytime'=>array('BETWEEN',"{$beforeTime},{$todayTime}")))
