...
class IndexController extends BaseController
{
    ...
    public function welcome()
    {
        ...
        if ($permissions) {
            ...
            $bookInfo = $bookModel->field('articleid,sumegold')->order('sumegold desc')->select();
            //分页
    ...
    public function newPayLog()
    {
        ...
        $cpid = cookie('cpid');
        if ($cpid && ($cpid > 0)) {
            $where['buytime'] = array('egt', $todayTime);
            $where['channelid'] = $channelid;
            $where['payflag'] = 1;
            $payLogSum = $paylog->where($where)->count();
        } else if ((cookie('permissions') == 0) && ($cpid == 0)) {
    ...
    public function ajaxPayLogList()
    {
        if (cookie('uid')) {
            ...
            if ($cpid && $cpid > 0) {
                ...
                $where['channelid'] = $channelID;
                $where['payflag'] = 1;
                $count = M('distribution_pay_paylog')->where($where)->count();
                ...
                $payLogList = $paypayLog->where($where)->limit($Page->firstRow . ',' . $Page->listRows)->order($orderStr)->select();
                ...
                if ($channelid) {
                    ...
                    $payIds = M('distribution_channels')->field('channelid')->where($where)->select();
                    $str = '';
                    foreach ($payIds as $v) {
                        $str .= ',' . $v['channelid'];
                    }
            ...
            foreach ($payLogList as $key => $item) {
                ...
                //充值总额
                $userSumPay = M('distribution_pay_paylog')->field('sum(money) as userSumPay')->where(array('buyid' => $payLogList[$key]['buyid'], 'payflag' => 1))->select();
                $userSumPay = $userSumPay[0];
                $payLogList[$key]['userSumPay'] = $userSumPay['userSumPay'];
