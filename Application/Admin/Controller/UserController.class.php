		...
		if(cookie('uid')){
			...
			//		var_dump($userInfo);
				...
				//	var_dump($channelids);exit;
					...
					$userChannel = M('distribution_channels')->field('pid,uid')->where(array('channelid'=>$userInfo['channelid']))->find();
						$channelInfo = M('system_users')->where(array('uid'=>$userChannel['uid']))->find();

						if($userChannel['pid']==0){
							$channelInfo['fchannel'] = $channelInfo['uname'];
						}else{
							$fchannel = M('distribution_channels')->field('uid,pid')->where(array('channelid'=>$userChannel['pid']))->find();
							$fuserInfo = M('system_users')->where(array('uid'=>$fchannel['uid']))->find();
							$channelInfo['fchannel'] = $fuserInfo['uname'];
						}
						$this->assign('channelInfo',$channelInfo);
						$this->assign('userinfo',$userInfo);
    ...
    public function editMyPass(){
    	...
    	if(IS_POST){
    		if (!empty($_POST['pass']) && !empty($_POST['uid'])) {
                ...
                if ($res !== false) {
                    $this->success('修改密码成功,请重新登录', U('/Admin/logout'));
                    exit;
                } else {
