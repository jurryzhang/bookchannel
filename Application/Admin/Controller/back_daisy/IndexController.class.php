...
class IndexController extends BaseController 
{
    ...
    public function getRoleMenu($permissions)
    {
    	...
    	$rs = M('distribution_system_module')->where('level > 1 AND visible = 1')->order('mod_id ASC')->select();
	...
	public function newPayLog()
	{
		...
		$payLogSum                = M('distribution_pay_paylog')->where($payLogContion)->count();
	...
	public function ajaxPayLogList()
	{
		if(cookie('uid'))
		{
			...
			$payLogList = M('distribution_pay_paylog')->where($payLogContion)->limit($Page->firstRow . ',' . $Page->listRows)->order($orderStr)->select();
