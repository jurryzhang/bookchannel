                ...
                <div class="panel-body">
                    ...
                    <div class="navbar navbar-default">
                        <form action="/index.php?s=/Channel/titleList/" id="search-form2" class="navbar-form form-inline" method="post" onsubmit="return false">
                                <button type="button" onclick="location.href='/index.php?s=/Channel/keywordAdd.html'" class="btn btn-primary pull-left">
                                    <i class="fa fa-plus"></i>添加关键词
                                </button>
                        </form>
                    </div>
                    <?php if(empty($keywordList)): ?><p class="goods_title">抱歉暂时没有相关结果！</p>
                        <?php else: ?>
                        <form method="post" enctype="multipart/form-data" target="_blank" id="form-pic">
                            <div class="table-responsive">
                                <table class="table table-bordered table-hover">
                                    ...
                                    <tbody>
                                    <?php if(is_array($keywordList)): $i = 0; $__LIST__ = $keywordList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                                            ...
                                            <td class="text-center">
                                                    ...
                                                    <a href="/index.php?s=/Channel/keywordEdit/id/<?php echo ($list["id"]); ?>.html"  title="" class="btn btn-success" data-original-title="编辑">
                                                        <span>编辑</span>
                                                    </a>
    ...
    function del(id,obj)
    {
        layer.confirm('确定要删除吗?',
            function(){
                $.ajax(
                    {
                        url     : "/index.php?s=/Channel/keywordDel/id/"+id,
                        success : function(v)
                        {
                            if(v.status==1){
