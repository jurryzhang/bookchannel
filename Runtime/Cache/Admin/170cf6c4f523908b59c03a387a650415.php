        ...
        <div class="container-fluid">
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <?php if(empty($urgeList)): ?><p class="goods_title">抱歉暂时没有相关结果！</p>
                        <?php else: ?>
                        <form method="post" enctype="multipart/form-data" target="_blank" id="form-pic">
                            <div class="table-responsive">
                                <table class="table table-bordered table-hover">
                                    ...
                                    <tbody>
                                    <?php if(is_array($urgeList)): $i = 0; $__LIST__ = $urgeList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr style="">
                                            ...
                                            <td class="replyBtn" data-id="<?php echo ($list['urgeid']); ?>" data-name="<?php echo ($list['username']); ?>" style="text-align:center;border-left: none;cursor: pointer;">
                                                <?php if($list['reply'] == ''): ?><i class="fa fa-comment-o"></i>
                                                    <?php else: ?>
                                                    <i class="fa fa-comment"></i><?php endif; ?>
                                                <input type="hidden" value="<?php echo ($list["reply"]); ?>" data-name="<?php echo ($list["username"]); ?>" id="reply<?php echo ($list['urgeid']); ?>"/>
                                            </td>
...
<script>
    ...
    $('.replyBtn').click(function(even){
        ...
        urgeid=$(this).attr('data-id');
        var content=$("#reply"+urgeid).val();
        $('#replyBox textarea').html(content);

        var username=$("#reply"+urgeid).attr('data-name');
        $('#replyHeader').html('回复 '+username);
        $('#submitBtn').click(function(){
            content=$("#content").val();
            $.ajax({
                type    : 'POST',
                url     : '/index.php?m=Admin&c=Doc&a=urgeReply',
                data    : {urgeid:urgeid,content:content},
                success : function(data){
                    if(data.status==1){
